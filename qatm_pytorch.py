    ...
    def __getitem__(self, idx):
        ...
        if self.thresh_df is not None:
            if self.thresh_df.path.isin([template_path]).sum() > 0:
                thresh = float(
                    self.thresh_df[self.thresh_df.path == template_path].thresh
                )
        return {
            "image": self.image,
...
class QATM:
    ...
    def __call__(self, x):
        ...
        ind1, ind2 = torch.meshgrid(
            torch.arange(batch_size), torch.arange(ref_row * ref_col)
        )
        ind1 = ind1.flatten()
        ind2 = ind2.flatten()
        ind3 = ind3.flatten()
...
def nms(score, w_ini, h_ini, thresh=0.7):
    ...
    """
    socre: np.array, shape=(H, W)
    w_ini: int, width of template
    h_ini: int, height of template
    thresh: float, threshold for nms
    return: np.array, shape=(N, 2, 2)    
    """
...
def nms_multi(scores, w_array, h_array, thresh_list):
    ...
    for index, score in zip(indices_omit, scores_omit):
        ...
        if dots is None:
            ...
            dots_indices = np.concatenate(
                [dots_indices, np.ones(dot.shape[-1]) * index], axis=0
            )
...
def plot_result_multi(
    image_raw, boxes, indices, show=False, save_name=None, color_list=None
):
    d_img = image_raw.copy()
    ...
    for i in range(len(indices)):
        d_img = plot_result(
            d_img, boxes[i][None, :, :].copy(), color=color_list[indices[i]]
        )
    if show:
        plt.imshow(d_img[:, :, ::-1])
    if save_name:
...
def run_one_sample(model, template, image, image_name):
    ...
    for i in range(batch_size):
        ...
        score = np.exp(
            score / (h * w)
        )  # reverse number range back after computing geometry average
        scores.append(score)
...
def run_multi_sample(model, dataset):
    ...
    for data in dataset:
        score = run_one_sample(
            model, data["template"], data["image"], data["image_name"]
        )
        scores.append(score)
...
#         Path(template_dir), image_path, thresh_csv="thresh_template.csv"
#     )
#     model = CreateModel(
#         model=models.vgg19(pretrained=True).features, alpha=25, use_cuda=True
#     )
